...
const register = async (req, res) => {
  ...
  //sending email for verification if user is created
...
// start of verify email controller

const verifyEmail = async (req, res) => {
  const { verificationToken, email } = req.body
  const user = await User.findOne({ email })

  //does the user email belong to a user in the database
  if (!user) {
    throw new CustomError.UnauthenticatedError("Verification Failed ")
  }

  //does the user verification Token match what we recieved
  if (user.verificationToken !== verificationToken) {
    throw new CustomError.UnauthenticatedError("Verification Failed ")
  }

  //if we have a user with the current email and verification token then the user is verified
...
const resetPassword = async (req, res) => {
  ...
  if (user) {
    ...
    if (
      ...
      user.passwordTokenExipirationDate = null
      await user.save()
    }
  }
